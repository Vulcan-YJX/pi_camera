...
char * PiCamera::getCameraId(){
    return cameraId.data();
}

void PiCamera::configureStill(int width, int height, PixelFormat format, int buffercount, int rotation) {
...
void PiCamera::requestComplete(Request *request) {
    if (request->status() == Request::RequestCancelled)
        return;
    processRequest(request);
}

void PiCamera::processRequest(Request *request) {
    requestQueue.push(request);
}

int PiCamera::startCapture() {
    int ret;
...
int PiCamera::queueRequest(Request *request) {
...
Stream *PiCamera::VideoStream(uint32_t *w, uint32_t *h, uint32_t *stride) const
{
	StreamDimensions(viewfinder_stream_, w, h, stride);
	return viewfinder_stream_;
}

void PiCamera::StreamDimensions(Stream const *stream, uint32_t *w, uint32_t *h, uint32_t *stride) const
{
...
bool PiCamera::readFrame(LibcameraOutData *frameData){
...
void PiCamera::returnFrameBuffer(LibcameraOutData frameData) {
...
void PiCamera::set(ControlList controls){
    std::lock_guard<std::mutex> lock(control_mutex_);
